!function() {
  ...
  const pipe = (f, ...fs) => Object.assign(
    (..._) => reduce(call2, fs, f(..._)),
    {
      catch(...cfs) {
        return intercept(f, ...fs).catch(...cfs);
      },
      finally(...cfs) {
        return intercept(f, ...fs).finally(...cfs);
      },
      intercept(...cfs) {
        return intercept(f, ...fs).intercept(...cfs);
      }
    }
  );
  ...
  function intercept(f, ...fs) {
    var catchF, finallyF = identity, interceptors = [];
    return Object.assign(function(...args) {
      var stop, catched;
      function _catched(e) {
        stop = true;
        return go(catched = catchF ? catchF(e) : console.error(e));
      }
      function body(f, ...args) {
        try {
          return go(
            find((interceptor) => interceptor.predi(...args), interceptors),
            function(interceptor) {
              if (interceptor) {
                stop = true;
                return catched = interceptor.body(...args);
              }
              if (stop) return catched;
              const arg = f(...args);
              return arg instanceof Promise ? arg.catch(_catched) : arg;
            }
          );
        } catch (e) {
          return _catched(e);
      ...
      return go(
        reduce((arg, f) => body(f, arg), fs, body(f, ...args)),
        function(res) {
          return stop ? finallyF(catched) : finallyF(res);
        }
      );
    }, {
      catch(...fs) {
      ...
      intercept(...fs) {
        var interceptor = { predi: pipe(...fs) };
        interceptors.push(interceptor);
        return (...fs) => {
          interceptor.body = pipe(...fs);
          return this;
        }
      }
    });
